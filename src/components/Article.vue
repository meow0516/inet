<template>
  <article class="flex justify-content-center mb-2">
    <div
      class="flex flex-column text-left border-round border-1 border-500 surface-0 article"
    >
      <router-link :to="{ path: RoutePath.article(id) }" class="mb-5">
        <img
          :src="coverImage"
          class="h-15rem w-full cover-image"
          alt="coverImage"
        />
      </router-link>
      <div class="flex px-5">
      ...
      <div class="article-info px-5">
        <router-link :to="{ path: RoutePath.article(id) }">
...
let { title, body, views, likes, createdAt, coverImage, id } = toRefs(
  article.value
);
let author = article.value.author.name || article.value.author.username;
let avatar = article.value.author.avatar;
let convertCreatedAt = computed(() =>
  moment(createdAt.value).format('YYYY-MM-DD HH:mm')
);
</script>
